...
class SettingObserver
{
	...
	private function regenerateLangRoutes($setting = null)
	{
		...
		try {
			...
			$origRoutes = $manager->getFileContent(config_path('larapen/routes/en.php'));
			
			// Get all the others languages
			$locales = $manager->getLocales();
			if (!empty($locales)) {
				foreach ($locales as $locale) {
					$transRoutesPath = config_path('larapen/routes/' . $locale . '.php');
					if (file_exists($transRoutesPath)) {
						$routes = $manager->getFileContent($transRoutesPath);
					} else {
						$routes = $origRoutes;
					}
