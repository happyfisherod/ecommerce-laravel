...
function qsUrl($path = null, $inputArray = [], $secure = null, $localized = true)
{
	if ($localized) {
		...
		if (currentLocaleShouldBeHiddenInUrl(config('app.locale'))) {
			if (!preg_match('#^http(s)?://#', $path)) {
				$localePath = config('app.locale') . '/';
				if (\Illuminate\Support\Str::startsWith($path, $localePath)) {
					$path = mb_substr($path, strlen($localePath));
				}
			}
		}
...
function lurl($path = null, $attributes = [], $locale = null)
{
	...
	if (request()->segment(1) == admin_uri()) {
		...
		$url = \Larapen\LaravelLocalization\Facades\LaravelLocalization::getLocalizedURL($locale, $path, $attributes);
	}
...
function appIsInstalled()
{
	...
	try {
		...
		if (preg_match('/.*(max_user_connections|max_connections|Too many connections).*/ui', $e->getMessage())) {
			dd($e->getMessage());
		}
		$properly = false;
	} catch (\Exception $e) {
...
function genPhoneNumberBtn($post, $btnBlock = false)
{
	...
	if (config('settings.single.hide_phone_number')) {
		...
		$btnLink = '';
		$btnAttrTooltip = 'data-toggle="tooltip" data-original-title="' . $btnHint . '"';
	...
	if (config('settings.single.show_security_tips') == '1') {
		...
		*/
		$btnAttrTooltip = 'rel="tooltip" data-original-title="' . $btnHint . '"';
		$btnClassTooltip = 'tooltipHere';
		$btnAttrModal = 'data-toggle="modal"';
	...
	if ($enableWhatsAppBtn) {
		$waBtnAttr = 'data-toggle="tooltip" data-original-title="' . t('chat_on_whatsapp') . '"';
